...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_create_recipe_with_tags(self):
        ...
        tag1 = sample_tag(user=self.user, name='Paszka')
        tag2 = sample_tag(user=self.user, name='Bugsy')
        payload = {
            'title': 'ChessBurger Kentucky',
    ...
    def test_create_recipe_with_ingredients(self):
        """Test creating recipe with ingriedients"""
        ingredient1 = sample_ingredient(user=self.user, name='Prawns')
        ingredient2 = sample_ingredient(user=self.user, name='lime')
        payload = {
            'title': 'Padthai with prawn',
            'ingriedients': [ingredient1.id, ingredient2.id],
            'time_minutes': 20,
            'price': 7.00
        }
        res = self.client.post(RECIPE_URL, payload)
        ...
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        recipe = Recipe.objects.get(id=res.data['id'])
        ingredients = recipe.ingriedients.all()
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload_image_to_recipe(self):
        """test uploading image too recipe"""
